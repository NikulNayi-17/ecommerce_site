...
def cart(request, total=0, quantity=0, grand_total=0, cart_items=None):
    try:
        tax = 0
        grand_total = 0
        if request.user.is_authenticated:
...
def checkout(request, total=0, quantity=0, grand_total=0, cart_items=None, tax=0):
    try:
        if request.user.is_authenticated:
            cart_items = CartItem.objects.filter(
                user=request.user, is_active=True)
